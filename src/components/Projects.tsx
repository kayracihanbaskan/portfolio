...
export function Projects() {
  ...
  return (
    <section id="projeler" className="scroll-mt-20 bg-slate-50/60">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {t.projects.items.map((p, i) => (
            <Reveal key={p.title} delay={(i % 2) * 80}>
              <article className="group flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-6 transition-all hover:-translate-y-1 hover:shadow-lg">
                ...
                <h3 className="font-heading mt-1 text-lg font-bold text-slate-900">{p.title}</h3>
                ...
                <div className="mt-5 flex items-center gap-4 border-t border-slate-100 pt-4">
                  <a
                    ...
                    className="inline-flex min-h-[44px] items-center gap-1.5 text-sm font-semibold text-slate-700 hover:text-indigo-600"
                  >
                    <GithubIcon size={16} />
                    {t.projects.viewCode}
                  </a>
